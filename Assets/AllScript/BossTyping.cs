...
public class BossTyping  : MonoBehaviour
{
    public PlayerController playerController;//プレイヤーの操作スクリプト参照
    public BossManager currentBoss;//今ターゲットにしている敵
    ...
    void Update()
    {
        ...
        // 敵がいる場合のタイピング処理
        //敵の名前が空なら何もしない
        if (string.IsNullOrEmpty(currentBoss.typingWord))
            return;
        ...
        foreach (char c in Input.inputString)//打った文字を１文字ずつ処理
        {
            ...
            //入力が正しい場合
            if (currentInput.ToLower() == currentBoss.typingWord.ToLower())//currentInput が typingWord と完全一致した場合
            {
                if (playerController != null)
                {
                    playerController.StartAimAssist(currentBoss.transform); //StartAimAssistを呼び出す
            ...
            else if (!currentBoss.typingWord.ToLower().StartsWith(currentInput.ToLower()))
            {
                currentInput = "";//間違えた瞬間リセット
            }

            if (currentBoss.typingText != null)
            {
                //入力された文字を赤くする
                string full = currentBoss.typingWord;
                ...
                currentBoss.typingText.text = display;
    ...
    {
        GameObject[] enemies = GameObject.FindGameObjectsWithTag("Boss");//Bossタグをつけられたオブジェクトをすべて参照
        float minDistance = Mathf.Infinity;//一番近い敵との距離を一時的に保存するための変数（最初は無限大)
        BossManager closest = null;//一番近い敵の EnemyManager を保持する変数

        foreach (GameObject enemy in enemies)//全ての敵に対して 1 つずつ処理を行うループ
        {
            BossManager manager = enemy.GetComponent<BossManager>();//各 enemy に EnemyManager コンポーネントが付いているか確認
        ...
        if (closest != null && currentBoss != closest)//最も近くにいる敵が null でない かつ、それが 今のターゲットではない（currentEnemyと違う）なら
        {
            SetTargetEnemy(closest);//ターゲットをその敵に更新する。
        }
    }

    public void SetTargetEnemy(BossManager enemy)
    {



        Debug.Log("SetTargetEnemy 呼び出し:" + enemy.name);
        if (currentBoss != null && currentBoss.HighrightLine != null)
        {
            currentBoss.HighrightLine.SetActive(false);// ハイライト非表示
        }

        currentBoss = enemy;//currentEnemyを更新

        if (currentBoss.HighrightLine != null)
        {
            currentBoss.HighrightLine.SetActive(true); // ハイライト表示
        }
        ...
        Debug.Log("セット後　currentEnemy=" + currentBoss);
    }
}
